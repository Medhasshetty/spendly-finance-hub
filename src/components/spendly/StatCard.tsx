...
export function StatCard({
  ...
  const accentStyles: Record<string, { icon: string; ring: string; gradient: string; text: string; badge: string }> = {
    neutral: {
  ...
  return (
    <div className="card-surface group relative overflow-hidden p-5 hover:-translate-y-0.5">
      <div
        aria-hidden="true"
      ...
      <div className="relative">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <p className="truncate text-[12px] font-medium uppercase tracking-wider text-muted-foreground">
              {label}
            </p>
          </div>
          <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-2xl ring-1 ${accentStyles.icon} transition-all duration-300 group-hover:scale-105 group-hover:shadow-lg ${accentStyles.ring}`}>
        ...
        {loading ? (
          ...
          <>
            <div className="mt-3 flex items-end justify-between gap-3">
              <p className={`num overflow-hidden text-3xl font-bold leading-none tracking-tight ${accentStyles.text} sm:text-[34px]`}>
                <span className="inline-block bg-gradient-to-r from-current via-current to-current/70 bg-clip-text text-transparent">
                  {value}
                </span>
              </p>
              {trend && (
                <span className={`inline-flex shrink-0 items-center gap-1 rounded-full border px-2 py-1 text-[11px] font-semibold ${accentStyles.badge}`}>
                  {trend.value}
                </span>
              )}
            </div>
